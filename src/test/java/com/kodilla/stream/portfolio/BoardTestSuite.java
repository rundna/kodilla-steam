...
public class BoardTestSuite {
    ...
    public void testAddTaskListAverageWorkingOnTask(){
        ...
        long longTasks = project.getTaskLists().stream()
                ...
                //.filter(r->r>0)
                //.map(t -> Duration.between(LocalDate.now().getDayOfMonth(), t.getCreated().getDayOfMonth()).toDays())
                //.forEach(System.out::println);
                //.collect(toList());
                //.filter(d -> Duration.between(LocalDate.now(),d))
                .reduce((long) 0,(sum, current)->sum+=current); //why to long?
        ...
        System.out.println(tasksInProgressCount);

        assertEquals(10,longTasks/tasksInProgressCount);
    }
    @Test
